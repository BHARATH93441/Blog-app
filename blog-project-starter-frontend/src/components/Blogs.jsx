...
function Blogs() {
    ...
    useEffect(() => {
        ...
        if(user)
        {
          if(user.uid === "blL5q0vGZ9fRv954IfgIqh90uB52")
          {
            setAdmin(true)
          }
          else{
        ...
        axios.get("https://blog-app-backend-z85y.onrender.com/api/blogs").then((res) => {
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`https://blog-app-backend-z85y.onrender.com/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("https://blog-app-backend-z85y.onrender.com/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
